...
public class SolutionTest {
    ...
    public void testConsumeAndStatistics() throws Exception {
        ...
        Mockito.verify(orderService, Mockito.times(1))
                .getAllOrders();
        Mockito.verify(inventoryService, Mockito.times(0))
                .update(Mockito.any(InventoryRequest.class));
    }

    @Test
    public void testConsumerAndStatisticsWithOrderNew() throws Exception {
        stubFor(get(urlPathEqualTo("/products"))
                .withQueryParam("page", equalTo("1"))
                .withQueryParam("brand_id", equalTo("b_d2481b88"))
                .willReturn(aResponse()
        ...
        stubFor(WireMock.post(urlPathEqualTo("/orders/bo_3s7ei5an/items/availability"))
                .withHeader(CONTENT_TYPE, CONTENT_TYPE_PATTERN)
                .withHeader(X_FAIRE_ACCESS_TOKEN, FAIRE_TOKEN)
                .willReturn(aResponse()
                        .withStatus(201)
        ...
        stubFor(put(urlPathEqualTo("/orders/bo_2ppdbqvn/processing"))
                .withHeader(CONTENT_TYPE, CONTENT_TYPE_PATTERN)
                .withHeader(X_FAIRE_ACCESS_TOKEN, FAIRE_TOKEN)
                .willReturn(aResponse()
                        .withStatus(200)
        ...
        Map<String, BackOrderItem> backOrderItemMap = new HashMap() {{
            put("oi_6x9ejf4p", new BackOrderItem(25));
            put("oi_a69xyy8e", new BackOrderItem(10));
        }};
        Mockito.verify(orderService, Mockito.times(1))
                .backOrderItem(Mockito.eq("bo_3s7ei5an"), Mockito.eq(backOrderItemMap));
        Mockito.verify(orderService, Mockito.times(1))
                .backOrderItem(Mockito.anyString(), Mockito.anyMap());
        Mockito.verify(orderService, Mockito.times(1))
                .getAllOrders();
        Mockito.verify(inventoryService, Mockito.times(1))
                .update(Mockito.any(InventoryRequest.class));
        Mockito.verify(orderService, Mockito.times(1))
                .acceptOrder(Mockito.eq("bo_2ppdbqvn"));
    }
    ...
    public void tesStatistics() throws Exception {
        ...
        mockMvc.perform(
                ...
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(jsonPath("best_selling_product_option.key", Matchers.is("po_6kkvedwj")))
                .andExpect(jsonPath("best_selling_product_option.value", Matchers.is(2)))
                .andExpect(jsonPath("largest_order_by_dollar_ammount.key", Matchers.is("bo_alanim4r")))
                .andExpect(jsonPath("largest_order_by_dollar_ammount.value", Matchers.is(25560)))
                .andExpect(jsonPath("state_most_order.key", Matchers.is("Florida")))
                .andExpect(jsonPath("state_most_order.value", Matchers.is(1)))
                .andExpect(jsonPath("best_selling_sku.key", Matchers.is("COMPF002")))
                .andExpect(jsonPath("best_selling_sku.value", Matchers.is(2)))
                .andExpect(jsonPath("state_most_amount_order.key", Matchers.is("South Carolina")))
                .andExpect(jsonPath("state_most_amount_order.value", Matchers.is(25560)));
